...
def make_points(image, line):
    if line is None:
        return None  # Handle the case where line is not valid
...
def average_slope_intercept(image, lines):
    ...
    if lines is None or len(lines) == 0:
        return None  # Handle the case where no lines are detected
    ...
    # Check if left_fit or right_fit are empty before averaging
    ...
    # Check if averages are valid before making points
...
def canny(img):
    ...
    canny = cv2.Canny(blur, 50, 150)  # Use blurred image for Canny
    return canny
...
def display_lines(img, lines):
    ...
    if lines is not None:
        for line in lines:
            if line is not None:  # Check if line is valid
                for x1, y1, x2, y2 in line:
